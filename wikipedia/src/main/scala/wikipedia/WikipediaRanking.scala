...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("coursera_scala_spark_wiki_app")
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    toResult { index.mapValues(values => values.aggregate(0)((sum, _) => sum + 1, _ + _)) }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    toResult { rdd.flatMap(a => langs.filter(a.mentionsLanguage).map((_, 1))).reduceByKey(_ + _) }
